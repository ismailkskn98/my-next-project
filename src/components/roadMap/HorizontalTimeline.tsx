  ...
  return (
    ...
    >
      <div className="relative w-full">
        <div className="bg-logoGold absolute left-0 right-0 top-1/2 h-0.5 w-full -translate-y-1/2 rounded-full" />
          ...
          {milestones.slice(0, 6).map((item, index) => (
            ...
            >
              <div className="absolute left-1/2 top-1/2 z-10 h-4 w-4 -translate-x-1/2 transform rounded-full bg-primary md:top-[calc(50%-0.5rem)]">
                <div className="2xl: bg-logoGold absolute left-1/2 top-1/2 h-2 w-2 -translate-x-1/2 -translate-y-1/2 transform rounded-full" />
              </div>

              <div
                className={cn(
                  "bg-logoGold absolute left-1/2 w-0.5",
                  index % 2 === 0 ? "bottom-[50%] h-5 md:h-5" : "top-[50%] h-5",
                )}
              />

              <Card className="relative min-h-[192px] transform border-none bg-white/10 text-white/90 transition-transform duration-300 hover:scale-105">
                <CardContent className="p-6">
                  <time className="text-logoGold mb-2 block text-sm">
                    {item.date}
                  </time>
